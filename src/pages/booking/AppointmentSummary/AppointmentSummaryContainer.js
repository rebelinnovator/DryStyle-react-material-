...
const AppointmentSummaryContainer = ({
    ...
    useEffect(() => {
        let newPrice = 0;
        newPrice += 10 * (addons?.[0]?.data?.length || 0);
        newPrice += (selectedServices?.[0]?.data?.Price?.Amount || 0);

        setEstimatedTotal(newPrice);
    ...
    return (
        // <div className={classes.appointmentSummaryDiv}>
        <Grid className={fullWidth ? classes.fullWidth : classes.container}>
            ...
            {
                // Date and time
                getSavedNoteMessage.length > 1
                    ? (
                        <SummaryCard classes={classes} editClickPath="/booking/notes" heading="Stylist">
                            <Typography>
                                {getSavedNoteMessage}
                            </Typography>
                        </SummaryCard>
